/*
Identity Security Cloud API - Entitlements

Use these APIs to interact with the Identity Security Cloud platform to achieve repeatable, automated processes with greater scalability. We encourage you to join the SailPoint Developer Community forum at https://developer.sailpoint.com/discuss to connect with other developers using our APIs.

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package entitlements

import (
	"encoding/json"
	"fmt"
)

// checks if the Entitlementmetadatabulkupdatebyfilterrequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &Entitlementmetadatabulkupdatebyfilterrequest{}

// Entitlementmetadatabulkupdatebyfilterrequest Request to bulk update Access Model Metadata on every entitlement matching a filter expression. Adding or replacing custom metadata requires a suite license.
type Entitlementmetadatabulkupdatebyfilterrequest struct {
	// Filter results using the standard syntax described in [V3 API Standard Collection Parameters](https://developer.sailpoint.com/idn/api/standard-collection-parameters#filtering-results)  Filtering is supported for the following fields and operators:  **id**: *eq*
	Filters string `json:"filters"`
	// The operation to be performed
	Operation string `json:"operation"`
	// The choice of update scope. **ATTRIBUTE** replaces only the values of the attributes named in `values`, and **ALL** replaces every metadata attribute on the entitlement.
	ReplaceScope *string `json:"replaceScope,omitempty"`
	// The metadata to be updated, including attribute key and value.
	Values []EntitlementmetadatabulkupdatebyidrequestValuesInner `json:"values"`
	AdditionalProperties map[string]interface{}
}

type _Entitlementmetadatabulkupdatebyfilterrequest Entitlementmetadatabulkupdatebyfilterrequest

// NewEntitlementmetadatabulkupdatebyfilterrequest instantiates a new Entitlementmetadatabulkupdatebyfilterrequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewEntitlementmetadatabulkupdatebyfilterrequest(filters string, operation string, values []EntitlementmetadatabulkupdatebyidrequestValuesInner) *Entitlementmetadatabulkupdatebyfilterrequest {
	this := Entitlementmetadatabulkupdatebyfilterrequest{}
	this.Filters = filters
	this.Operation = operation
	this.Values = values
	return &this
}

// NewEntitlementmetadatabulkupdatebyfilterrequestWithDefaults instantiates a new Entitlementmetadatabulkupdatebyfilterrequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewEntitlementmetadatabulkupdatebyfilterrequestWithDefaults() *Entitlementmetadatabulkupdatebyfilterrequest {
	this := Entitlementmetadatabulkupdatebyfilterrequest{}
	return &this
}

// GetFilters returns the Filters field value
func (o *Entitlementmetadatabulkupdatebyfilterrequest) GetFilters() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Filters
}

// GetFiltersOk returns a tuple with the Filters field value
// and a boolean to check if the value has been set.
func (o *Entitlementmetadatabulkupdatebyfilterrequest) GetFiltersOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Filters, true
}

// SetFilters sets field value
func (o *Entitlementmetadatabulkupdatebyfilterrequest) SetFilters(v string) {
	o.Filters = v
}

// GetOperation returns the Operation field value
func (o *Entitlementmetadatabulkupdatebyfilterrequest) GetOperation() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Operation
}

// GetOperationOk returns a tuple with the Operation field value
// and a boolean to check if the value has been set.
func (o *Entitlementmetadatabulkupdatebyfilterrequest) GetOperationOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Operation, true
}

// SetOperation sets field value
func (o *Entitlementmetadatabulkupdatebyfilterrequest) SetOperation(v string) {
	o.Operation = v
}

// GetReplaceScope returns the ReplaceScope field value if set, zero value otherwise.
func (o *Entitlementmetadatabulkupdatebyfilterrequest) GetReplaceScope() string {
	if o == nil || IsNil(o.ReplaceScope) {
		var ret string
		return ret
	}
	return *o.ReplaceScope
}

// GetReplaceScopeOk returns a tuple with the ReplaceScope field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Entitlementmetadatabulkupdatebyfilterrequest) GetReplaceScopeOk() (*string, bool) {
	if o == nil || IsNil(o.ReplaceScope) {
		return nil, false
	}
	return o.ReplaceScope, true
}

// HasReplaceScope returns a boolean if a field has been set.
func (o *Entitlementmetadatabulkupdatebyfilterrequest) HasReplaceScope() bool {
	if o != nil && !IsNil(o.ReplaceScope) {
		return true
	}

	return false
}

// SetReplaceScope gets a reference to the given string and assigns it to the ReplaceScope field.
func (o *Entitlementmetadatabulkupdatebyfilterrequest) SetReplaceScope(v string) {
	o.ReplaceScope = &v
}

// GetValues returns the Values field value
func (o *Entitlementmetadatabulkupdatebyfilterrequest) GetValues() []EntitlementmetadatabulkupdatebyidrequestValuesInner {
	if o == nil {
		var ret []EntitlementmetadatabulkupdatebyidrequestValuesInner
		return ret
	}

	return o.Values
}

// GetValuesOk returns a tuple with the Values field value
// and a boolean to check if the value has been set.
func (o *Entitlementmetadatabulkupdatebyfilterrequest) GetValuesOk() ([]EntitlementmetadatabulkupdatebyidrequestValuesInner, bool) {
	if o == nil {
		return nil, false
	}
	return o.Values, true
}

// SetValues sets field value
func (o *Entitlementmetadatabulkupdatebyfilterrequest) SetValues(v []EntitlementmetadatabulkupdatebyidrequestValuesInner) {
	o.Values = v
}

func (o Entitlementmetadatabulkupdatebyfilterrequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o Entitlementmetadatabulkupdatebyfilterrequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["filters"] = o.Filters
	toSerialize["operation"] = o.Operation
	if !IsNil(o.ReplaceScope) {
		toSerialize["replaceScope"] = o.ReplaceScope
	}
	toSerialize["values"] = o.Values

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *Entitlementmetadatabulkupdatebyfilterrequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"filters",
		"operation",
		"values",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varEntitlementmetadatabulkupdatebyfilterrequest := _Entitlementmetadatabulkupdatebyfilterrequest{}

	err = json.Unmarshal(data, &varEntitlementmetadatabulkupdatebyfilterrequest)

	if err != nil {
		return err
	}

	*o = Entitlementmetadatabulkupdatebyfilterrequest(varEntitlementmetadatabulkupdatebyfilterrequest)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "filters")
		delete(additionalProperties, "operation")
		delete(additionalProperties, "replaceScope")
		delete(additionalProperties, "values")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableEntitlementmetadatabulkupdatebyfilterrequest struct {
	value *Entitlementmetadatabulkupdatebyfilterrequest
	isSet bool
}

func (v NullableEntitlementmetadatabulkupdatebyfilterrequest) Get() *Entitlementmetadatabulkupdatebyfilterrequest {
	return v.value
}

func (v *NullableEntitlementmetadatabulkupdatebyfilterrequest) Set(val *Entitlementmetadatabulkupdatebyfilterrequest) {
	v.value = val
	v.isSet = true
}

func (v NullableEntitlementmetadatabulkupdatebyfilterrequest) IsSet() bool {
	return v.isSet
}

func (v *NullableEntitlementmetadatabulkupdatebyfilterrequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableEntitlementmetadatabulkupdatebyfilterrequest(val *Entitlementmetadatabulkupdatebyfilterrequest) *NullableEntitlementmetadatabulkupdatebyfilterrequest {
	return &NullableEntitlementmetadatabulkupdatebyfilterrequest{value: val, isSet: true}
}

func (v NullableEntitlementmetadatabulkupdatebyfilterrequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableEntitlementmetadatabulkupdatebyfilterrequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


