...
func main() {
    ...
    resp, r, err := apiClient.V2026.EntitlementsAPI.GetAccountEntitlements(context.Background(), accountId).Execute()
	  //resp, r, err := apiClient.V2026.EntitlementsAPI.GetAccountEntitlements(context.Background(), accountId).Limit(limit).Offset(offset).Count(count).SearchAfter(searchAfter).Sorters(sorters).Execute()
    if err != nil {
