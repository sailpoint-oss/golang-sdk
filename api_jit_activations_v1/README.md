# Go API client for api_jit_activations_v1
...
```go
import api_jit_activations_v1 "github.com/sailpoint-oss/golang-sdk/v2/api_jit_activations_v1"
```
...
For using other server than the one defined on index 0 set context value `api_jit_activations_v1.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), api_jit_activations_v1.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `api_jit_activations_v1.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), api_jit_activations_v1.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `api_jit_activations_v1.ContextOperationServerIndices` and `api_jit_activations_v1.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), api_jit_activations_v1.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), api_jit_activations_v1.ContextOperationServerVariables, map[string]map[string]string{
