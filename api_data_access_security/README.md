# Go API client for api_data_access_security
...
```go
import api_data_access_security "github.com/sailpoint-oss/golang-sdk/v3/api_data_access_security"
```
...
For using other server than the one defined on index 0 set context value `api_data_access_security.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), api_data_access_security.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `api_data_access_security.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), api_data_access_security.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `api_data_access_security.ContextOperationServerIndices` and `api_data_access_security.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), api_data_access_security.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), api_data_access_security.ContextOperationServerVariables, map[string]map[string]string{
