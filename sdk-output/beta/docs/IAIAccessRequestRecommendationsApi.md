...
------------- | ------------- | -------------
[**AccessRequestRecommendations**](IAIAccessRequestRecommendationsApi.md#AccessRequestRecommendations) | **Get** /ai-access-request-recommendations | Identity Access Request Recommendations
...
## AccessRequestRecommendations

> []AccessRequestRecommendationItemDetail AccessRequestRecommendations(ctx).IdentityId(identityId).Limit(limit).Offset(offset).Count(count).IncludeTranslationMessages(includeTranslationMessages).Filters(filters).Sorters(sorters).Execute()
...
func main() {
    ...
    resp, r, err := apiClient.IAIAccessRequestRecommendationsApi.AccessRequestRecommendations(context.Background()).IdentityId(identityId).Limit(limit).Offset(offset).Count(count).IncludeTranslationMessages(includeTranslationMessages).Filters(filters).Sorters(sorters).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `IAIAccessRequestRecommendationsApi.AccessRequestRecommendations``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `AccessRequestRecommendations`: []AccessRequestRecommendationItemDetail
    fmt.Fprintf(os.Stdout, "Response from `IAIAccessRequestRecommendationsApi.AccessRequestRecommendations`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiAccessRequestRecommendationsRequest struct via the builder pattern
