# Go API client for api_notifications
...
```go
import api_notifications "github.com/sailpoint-oss/golang-sdk/v2/api_notifications"
```
...
For using other server than the one defined on index 0 set context value `api_notifications.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), api_notifications.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `api_notifications.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), api_notifications.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `api_notifications.ContextOperationServerIndices` and `api_notifications.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), api_notifications.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), api_notifications.ContextOperationServerVariables, map[string]map[string]string{
