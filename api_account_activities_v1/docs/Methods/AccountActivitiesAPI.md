...
keywords: ['go', 'Golang', 'sdk', 'AccountActivities', 'V1AccountActivities'] 
slug: /tools/sdk/go/accountactivitiesv1/methods/account-activities
...
Other parameters are passed through a pointer to a apiGetAccountActivityV1Request struct via the builder pattern
...
import (
	...
	sailpoint "github.com/sailpoint-oss/golang-sdk/v2/api_account_activities_v1"
)
...
func main() {
    ...
    resp, r, err := apiClient.AccountActivitiesV1.AccountActivitiesAPI.GetAccountActivityV1(context.Background(), id).Execute()
	  //resp, r, err := apiClient.AccountActivitiesV1.AccountActivitiesAPI.GetAccountActivityV1(context.Background(), id).Execute()
    if err != nil {
...
Other parameters are passed through a pointer to a apiListAccountActivitiesV1Request struct via the builder pattern
...
func main() {
    ...
    configuration := sailpoint.NewDefaultConfiguration()
    apiClient := sailpoint.NewAPIClient(configuration)
    resp, r, err := apiClient.AccountActivitiesV1.AccountActivitiesAPI.ListAccountActivitiesV1(context.Background()).Execute()
	  //resp, r, err := apiClient.AccountActivitiesV1.AccountActivitiesAPI.ListAccountActivitiesV1(context.Background()).RequestedFor(requestedFor).RequestedBy(requestedBy).RegardingIdentity(regardingIdentity).Limit(limit).Offset(offset).Count(count).Filters(filters).Sorters(sorters).Execute()
    if err != nil {
