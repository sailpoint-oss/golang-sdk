...
keywords: ['go', 'Golang', 'sdk', 'PublicIdentities', 'V1PublicIdentities'] 
slug: /tools/sdk/go/publicidentitiesv1/methods/public-identities
...
Other parameters are passed through a pointer to a apiGetPublicIdentitiesV1Request struct via the builder pattern
...
import (
	...
	sailpoint "github.com/sailpoint-oss/golang-sdk/v2/api_public_identities_v1"
)
...
func main() {
    ...
    resp, r, err := apiClient.PublicIdentitiesV1.PublicIdentitiesAPI.GetPublicIdentitiesV1(context.Background()).Execute()
	  //resp, r, err := apiClient.PublicIdentitiesV1.PublicIdentitiesAPI.GetPublicIdentitiesV1(context.Background()).Limit(limit).Offset(offset).Count(count).Filters(filters).AddCoreFilters(addCoreFilters).Sorters(sorters).Execute()
    if err != nil {
