# Go API client for machine_account_mappings
...
```go
import machine_account_mappings "github.com/sailpoint-oss/golang-sdk/v3/machine_account_mappings"
```
...
For using other server than the one defined on index 0 set context value `machine_account_mappings.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), machine_account_mappings.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `machine_account_mappings.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), machine_account_mappings.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `machine_account_mappings.ContextOperationServerIndices` and `machine_account_mappings.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), machine_account_mappings.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), machine_account_mappings.ContextOperationServerVariables, map[string]map[string]string{
