/*
NERM API v2025

The NERM API v2025 accesss and modifies resources in your environment.

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package api_nerm_v2025

import (
	"encoding/json"
	
)

// checks if the Delegation1 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &Delegation1{}

// Delegation1 struct for Delegation1
type Delegation1 struct {
	// The id of the delegation
	Id *string `json:"id,omitempty"`
	// The id of the delegator user
	DelegatorId map[string]interface{} `json:"delegator_id,omitempty"`
	// The id of the delegate user
	DelegateId map[string]interface{} `json:"delegate_id,omitempty"`
	// The expiration date of the delegation
	Expiration *SailPointTime `json:"expiration,omitempty"`
	// Indicates if the delegation is expired
	Expired *bool `json:"expired,omitempty"`
	// The date-time the record created.
	CreatedAt *SailPointTime `json:"created_at,omitempty"`
	// The date-time the record was last updated.
	UpdatedAt *SailPointTime `json:"updated_at,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _Delegation1 Delegation1

// NewDelegation1 instantiates a new Delegation1 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDelegation1() *Delegation1 {
	this := Delegation1{}
	return &this
}

// NewDelegation1WithDefaults instantiates a new Delegation1 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDelegation1WithDefaults() *Delegation1 {
	this := Delegation1{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *Delegation1) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Delegation1) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *Delegation1) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *Delegation1) SetId(v string) {
	o.Id = &v
}

// GetDelegatorId returns the DelegatorId field value if set, zero value otherwise.
func (o *Delegation1) GetDelegatorId() map[string]interface{} {
	if o == nil || IsNil(o.DelegatorId) {
		var ret map[string]interface{}
		return ret
	}
	return o.DelegatorId
}

// GetDelegatorIdOk returns a tuple with the DelegatorId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Delegation1) GetDelegatorIdOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.DelegatorId) {
		return map[string]interface{}{}, false
	}
	return o.DelegatorId, true
}

// HasDelegatorId returns a boolean if a field has been set.
func (o *Delegation1) HasDelegatorId() bool {
	if o != nil && !IsNil(o.DelegatorId) {
		return true
	}

	return false
}

// SetDelegatorId gets a reference to the given map[string]interface{} and assigns it to the DelegatorId field.
func (o *Delegation1) SetDelegatorId(v map[string]interface{}) {
	o.DelegatorId = v
}

// GetDelegateId returns the DelegateId field value if set, zero value otherwise.
func (o *Delegation1) GetDelegateId() map[string]interface{} {
	if o == nil || IsNil(o.DelegateId) {
		var ret map[string]interface{}
		return ret
	}
	return o.DelegateId
}

// GetDelegateIdOk returns a tuple with the DelegateId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Delegation1) GetDelegateIdOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.DelegateId) {
		return map[string]interface{}{}, false
	}
	return o.DelegateId, true
}

// HasDelegateId returns a boolean if a field has been set.
func (o *Delegation1) HasDelegateId() bool {
	if o != nil && !IsNil(o.DelegateId) {
		return true
	}

	return false
}

// SetDelegateId gets a reference to the given map[string]interface{} and assigns it to the DelegateId field.
func (o *Delegation1) SetDelegateId(v map[string]interface{}) {
	o.DelegateId = v
}

// GetExpiration returns the Expiration field value if set, zero value otherwise.
func (o *Delegation1) GetExpiration() SailPointTime {
	if o == nil || IsNil(o.Expiration) {
		var ret SailPointTime
		return ret
	}
	return *o.Expiration
}

// GetExpirationOk returns a tuple with the Expiration field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Delegation1) GetExpirationOk() (*SailPointTime, bool) {
	if o == nil || IsNil(o.Expiration) {
		return nil, false
	}
	return o.Expiration, true
}

// HasExpiration returns a boolean if a field has been set.
func (o *Delegation1) HasExpiration() bool {
	if o != nil && !IsNil(o.Expiration) {
		return true
	}

	return false
}

// SetExpiration gets a reference to the given SailPointTime and assigns it to the Expiration field.
func (o *Delegation1) SetExpiration(v SailPointTime) {
	o.Expiration = &v
}

// GetExpired returns the Expired field value if set, zero value otherwise.
func (o *Delegation1) GetExpired() bool {
	if o == nil || IsNil(o.Expired) {
		var ret bool
		return ret
	}
	return *o.Expired
}

// GetExpiredOk returns a tuple with the Expired field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Delegation1) GetExpiredOk() (*bool, bool) {
	if o == nil || IsNil(o.Expired) {
		return nil, false
	}
	return o.Expired, true
}

// HasExpired returns a boolean if a field has been set.
func (o *Delegation1) HasExpired() bool {
	if o != nil && !IsNil(o.Expired) {
		return true
	}

	return false
}

// SetExpired gets a reference to the given bool and assigns it to the Expired field.
func (o *Delegation1) SetExpired(v bool) {
	o.Expired = &v
}

// GetCreatedAt returns the CreatedAt field value if set, zero value otherwise.
func (o *Delegation1) GetCreatedAt() SailPointTime {
	if o == nil || IsNil(o.CreatedAt) {
		var ret SailPointTime
		return ret
	}
	return *o.CreatedAt
}

// GetCreatedAtOk returns a tuple with the CreatedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Delegation1) GetCreatedAtOk() (*SailPointTime, bool) {
	if o == nil || IsNil(o.CreatedAt) {
		return nil, false
	}
	return o.CreatedAt, true
}

// HasCreatedAt returns a boolean if a field has been set.
func (o *Delegation1) HasCreatedAt() bool {
	if o != nil && !IsNil(o.CreatedAt) {
		return true
	}

	return false
}

// SetCreatedAt gets a reference to the given SailPointTime and assigns it to the CreatedAt field.
func (o *Delegation1) SetCreatedAt(v SailPointTime) {
	o.CreatedAt = &v
}

// GetUpdatedAt returns the UpdatedAt field value if set, zero value otherwise.
func (o *Delegation1) GetUpdatedAt() SailPointTime {
	if o == nil || IsNil(o.UpdatedAt) {
		var ret SailPointTime
		return ret
	}
	return *o.UpdatedAt
}

// GetUpdatedAtOk returns a tuple with the UpdatedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Delegation1) GetUpdatedAtOk() (*SailPointTime, bool) {
	if o == nil || IsNil(o.UpdatedAt) {
		return nil, false
	}
	return o.UpdatedAt, true
}

// HasUpdatedAt returns a boolean if a field has been set.
func (o *Delegation1) HasUpdatedAt() bool {
	if o != nil && !IsNil(o.UpdatedAt) {
		return true
	}

	return false
}

// SetUpdatedAt gets a reference to the given SailPointTime and assigns it to the UpdatedAt field.
func (o *Delegation1) SetUpdatedAt(v SailPointTime) {
	o.UpdatedAt = &v
}

func (o Delegation1) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o Delegation1) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.DelegatorId) {
		toSerialize["delegator_id"] = o.DelegatorId
	}
	if !IsNil(o.DelegateId) {
		toSerialize["delegate_id"] = o.DelegateId
	}
	if !IsNil(o.Expiration) {
		toSerialize["expiration"] = o.Expiration
	}
	if !IsNil(o.Expired) {
		toSerialize["expired"] = o.Expired
	}
	if !IsNil(o.CreatedAt) {
		toSerialize["created_at"] = o.CreatedAt
	}
	if !IsNil(o.UpdatedAt) {
		toSerialize["updated_at"] = o.UpdatedAt
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *Delegation1) UnmarshalJSON(data []byte) (err error) {
	varDelegation1 := _Delegation1{}

	err = json.Unmarshal(data, &varDelegation1)

	if err != nil {
		return err
	}

	*o = Delegation1(varDelegation1)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "id")
		delete(additionalProperties, "delegator_id")
		delete(additionalProperties, "delegate_id")
		delete(additionalProperties, "expiration")
		delete(additionalProperties, "expired")
		delete(additionalProperties, "created_at")
		delete(additionalProperties, "updated_at")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableDelegation1 struct {
	value *Delegation1
	isSet bool
}

func (v NullableDelegation1) Get() *Delegation1 {
	return v.value
}

func (v *NullableDelegation1) Set(val *Delegation1) {
	v.value = val
	v.isSet = true
}

func (v NullableDelegation1) IsSet() bool {
	return v.isSet
}

func (v *NullableDelegation1) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDelegation1(val *Delegation1) *NullableDelegation1 {
	return &NullableDelegation1{value: val, isSet: true}
}

func (v NullableDelegation1) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDelegation1) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


