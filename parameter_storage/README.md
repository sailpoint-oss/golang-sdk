# Go API client for parameter_storage
...
```go
import parameter_storage "github.com/sailpoint-oss/golang-sdk/v3/parameter_storage"
```
...
For using other server than the one defined on index 0 set context value `parameter_storage.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), parameter_storage.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `parameter_storage.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), parameter_storage.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `parameter_storage.ContextOperationServerIndices` and `parameter_storage.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), parameter_storage.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), parameter_storage.ContextOperationServerVariables, map[string]map[string]string{
