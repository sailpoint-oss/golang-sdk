...
> []SlimAccount ListAccounts(ctx).DetailLevel(detailLevel).Limit(limit).Offset(offset).Count(count).Filters(filters).Execute()
...
func main() {
    ...
    // response from `ListAccounts`: []SlimAccount
