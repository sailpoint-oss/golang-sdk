...
keywords: ['go', 'Golang', 'sdk', 'AccountUsages', 'V1AccountUsages'] 
slug: /tools/sdk/go/accountusagesv1/methods/account-usages
...
Other parameters are passed through a pointer to a apiGetUsagesByAccountIdV1Request struct via the builder pattern
...
import (
	...
	sailpoint "github.com/sailpoint-oss/golang-sdk/v2/api_account_usages_v1"
)
...
func main() {
    ...
    resp, r, err := apiClient.AccountUsagesV1.AccountUsagesAPI.GetUsagesByAccountIdV1(context.Background(), accountId).Execute()
	  //resp, r, err := apiClient.AccountUsagesV1.AccountUsagesAPI.GetUsagesByAccountIdV1(context.Background(), accountId).Limit(limit).Offset(offset).Count(count).Sorters(sorters).Execute()
    if err != nil {
