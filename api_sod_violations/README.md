# Go API client for api_sod_violations
...
```go
import api_sod_violations "github.com/sailpoint-oss/golang-sdk/v2/api_sod_violations"
```
...
For using other server than the one defined on index 0 set context value `api_sod_violations.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), api_sod_violations.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `api_sod_violations.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), api_sod_violations.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `api_sod_violations.ContextOperationServerIndices` and `api_sod_violations.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), api_sod_violations.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), api_sod_violations.ContextOperationServerVariables, map[string]map[string]string{
