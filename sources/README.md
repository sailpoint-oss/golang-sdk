...
```go
import sources "github.com/sailpoint-oss/golang-sdk/v3/sources"
```
...
For using other server than the one defined on index 0 set context value `sources.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), sources.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `sources.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), sources.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `sources.ContextOperationServerIndices` and `sources.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), sources.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), sources.ContextOperationServerVariables, map[string]map[string]string{
...
## Documentation For Models
 ...
 - [SourceEntitlementAccessRequestConfig](docs/SourceEntitlementAccessRequestConfig.md)
 - [SourceEntitlementAccessRequestConfigMaxPermittedAccessDuration](docs/SourceEntitlementAccessRequestConfigMaxPermittedAccessDuration.md)
 - [SourceEntitlementApprovalScheme](docs/SourceEntitlementApprovalScheme.md)
 - [SourceEntitlementRequestConfig](docs/SourceEntitlementRequestConfig.md)
 - [SourceEntitlementRevocationRequestConfig](docs/SourceEntitlementRevocationRequestConfig.md)
