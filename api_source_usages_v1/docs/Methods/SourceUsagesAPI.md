...
keywords: ['go', 'Golang', 'sdk', 'SourceUsages', 'V1SourceUsages'] 
slug: /tools/sdk/go/sourceusagesv1/methods/source-usages
...
Other parameters are passed through a pointer to a apiGetStatusBySourceIdV1Request struct via the builder pattern
...
import (
	...
	sailpoint "github.com/sailpoint-oss/golang-sdk/v2/api_source_usages_v1"
)
...
func main() {
    ...
    resp, r, err := apiClient.SourceUsagesV1.SourceUsagesAPI.GetStatusBySourceIdV1(context.Background(), sourceId).Execute()
	  //resp, r, err := apiClient.SourceUsagesV1.SourceUsagesAPI.GetStatusBySourceIdV1(context.Background(), sourceId).Execute()
    if err != nil {
...
Other parameters are passed through a pointer to a apiGetUsagesBySourceIdV1Request struct via the builder pattern
...
func main() {
    ...
    configuration := sailpoint.NewDefaultConfiguration()
    apiClient := sailpoint.NewAPIClient(configuration)
    resp, r, err := apiClient.SourceUsagesV1.SourceUsagesAPI.GetUsagesBySourceIdV1(context.Background(), sourceId).Execute()
	  //resp, r, err := apiClient.SourceUsagesV1.SourceUsagesAPI.GetUsagesBySourceIdV1(context.Background(), sourceId).Limit(limit).Offset(offset).Count(count).Sorters(sorters).Execute()
    if err != nil {
