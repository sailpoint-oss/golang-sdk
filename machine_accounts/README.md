# Go API client for machine_accounts
...
```go
import machine_accounts "github.com/sailpoint-oss/golang-sdk/v3/machine_accounts"
```
...
For using other server than the one defined on index 0 set context value `machine_accounts.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), machine_accounts.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `machine_accounts.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), machine_accounts.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `machine_accounts.ContextOperationServerIndices` and `machine_accounts.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), machine_accounts.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), machine_accounts.ContextOperationServerVariables, map[string]map[string]string{
