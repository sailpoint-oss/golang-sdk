...
> []ListIdentityAccessItems200ResponseInner ListIdentityAccessItems(ctx, id).XSailPointExperimental(xSailPointExperimental).Type_(type_).Limit(limit).Count(count).Offset(offset).Execute()
